...
const rateLimit = {
  ...
  canMakeRequest() {
    ...
    return this.requests.length < this.maxPerMinute ? (this.requests.push(now), true) : false;
  },
  async waitIfNeeded() { if (!this.canMakeRequest()) await new Promise(r => setTimeout(r, 2000)); }
};

// ======== DATA LOADING (FIXED: Use Complete Data) =========
// Menggunakan logic server.js untuk membaca data lengkap
...
// ======== FUNGSI PENCARIAN KEYWORD (LOGIC SERVER.JS) =========
// Menggunakan bobot skor dari server.js agar akurat
...
const chatHandler = async (req, res) => {
  ...
  // SYSTEM PROMPT (SAMA PERSIS SERVER.JS)
...
2. Jika data referensi mengatakan layanan online tersedia, JAWAB BISA.
...
${grounding ? '\n📚 DATA REFERENSI (WAJIB DIGUNAKAN):\n' + grounding : ''}`;
  ...
  try {
    ...
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=PLACEHOLDER`;
...
// ======== UI CHAT (KODE LAMA ANDA - TIDAK DIUBAH) =========
app.get('/ui', (req, res) => {
...
<body>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Gunakan /chat agar kompatibel dengan Vercel dan Local
      const API_URL = '/chat'; 
